...
面向**库使用者**：把技能管理能力嵌入自有 Rust 工具。功能概览与 CLI 用法见
[README](../README.md)，命令行参考见 [CLI.md](CLI.md)，本项目开发者见
...
注入的路径用于沙箱/测试，避免触碰真实 home/config/cwd。
...
库刻意保持**纯数据**：从不打印、从不调用 `process::exit`；结果结构化，错误通过
`Result` 上抛。渲染与退出码完全由调用方（如 CLI）决定。库**无遥测**——不会有任何
数据离开你的机器。
